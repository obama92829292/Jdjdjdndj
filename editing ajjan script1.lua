...
createButton(tabFrames.Main, "Anti Steal", 410, function()
    ...
    local CarryAnim = ReplicatedStorage:WaitForChild("Animations"):WaitForChild("Player"):WaitForChild("Carry")
    local ExpectedCarryId = CarryAnim.AnimationId
    ...
    local function teleportBehindFacingPlayer(targetPlayer)
        ...
        local behindPos = targetRoot.Position - targetRoot.CFrame.LookVector * 0.5
    ...
    local function hookPlayer(plr)
        ...
        local function onCharacter(char)
            ...
            local animConn = animator.AnimationPlayed:Connect(function(track)
                if track.Animation.AnimationId == ExpectedCarryId and not tracking[plr] and not currentTarget and isTrackerEnabled then
                    tracking[plr] = true
                    currentTarget = plr

                    notify("Tracking " .. plr.Name .. "\nBrainrot detected.", "TRACKING STARTED")

                    task.spawn(function()
                        while tracking[plr] and currentTarget == plr and isTrackerEnabled do
                            teleportBehindFacingPlayer(plr)

                            local stillHolding = false
                            local humanoid = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
                            if humanoid then
                                for _, anim in ipairs(humanoid:GetPlayingAnimationTracks()) do
                                    if anim.Animation.AnimationId == ExpectedCarryId then
                                        stillHolding = true
                                    end
                                end
                            end

                            if not stillHolding then break end
                            task.wait(0.3)
                        end
                        ...
                        if currentTarget == plr then
                            currentTarget = nil
                            notify(plr.Name .. " dropped the Brainrot.\nReady to track next.", "TARGET CLEARED")
                        end
                    end)
                end
            end)
            table.insert(trackerConnections, animConn)
        end

        if plr.Character then onCharacter(plr.Character) end
        table.insert(trackerConnections, plr.CharacterAdded:Connect(onCharacter))
    end

    if isTrackerEnabled then
        notify("Brainrot Tracker enabled.", "AJJANS TRACKER")
        for _, plr in ipairs(Players:GetPlayers()) do hookPlayer(plr) end
        table.insert(trackerConnections, Players.PlayerAdded:Connect(hookPlayer))
    else
        for _, conn in ipairs(trackerConnections) do conn:Disconnect() end
        trackerConnections = {}
        notify("Brainrot Tracker disabled.", "AJJANS TRACKER")
    end
end)
